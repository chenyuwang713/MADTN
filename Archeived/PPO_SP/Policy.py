...
class PPO(object):
    ...
    #         if stage == 'sample':
    #             pi = self.actor_s.pi(state)
    #             a = torch.round(pi).cpu().numpy().flatten()
    #             return a, 1.0
    #         elif stage == 'push':
    #             state = state.reshape(1, -1).to(torch.float).to(device)
    ...
    def train(self):
        s, a_s, a_p, r_s, r_p, s_prime, old_log_prob_s, old_log_prob_p, terminal = self.make_batch()
    ...
    def make_batch(self):
        s_lst, a_s_lst, a_p_lst, r_s_lst, r_p_lst, s_prime_lst, old_log_prob_s_lst, old_log_prob_p_lst, done_lst = [], [], [], [], [], [], [], [], []

        for transition in self.data:
            s, a_s, a_p, r_s, r_p, s_prime, old_log_prob_s, old_log_prob_p, done = transition
            s_lst.append(s)
            a_s_lst.append(a_s)
            a_p_lst.append(a_p)
            r_s_lst.append([r_s])
        ...
        '''list to tensor'''
        with torch.no_grad():
            s, a_s, a_p, r_s, r_p, s_prime, old_log_prob_s, old_log_prob_p, done_mask = \
        ...
        return s, a_s, a_p, r_s, r_p, s_prime, old_log_prob_s, old_log_prob_p, done_mask
