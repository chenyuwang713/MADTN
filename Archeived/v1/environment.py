...
def action_map(x, dev):
    num = np.zeros(dev)
    r = np.zeros(dev + 1)
    r[0] = x
    for i in range(dev):
        num[dev - i - 1] = r[i] // (2 ** (dev - 1 - i))
        r[i + 1] = r[i] % (2 ** (dev - 1 - i))
...
class Service():
    ...
    def get_priority(self):
        return np.array(self.priority) / np.sum(self.priority) 
...
def main():
    ...
    for i in range(502):
        ...
        print(env.service.get_channel_num())
        _, _, _, _, info =env.step( np.array([random.randint(0, 1) for _ in range(env.device_num)]), np.array([random.random() for _ in range(env.device_num)] ))
