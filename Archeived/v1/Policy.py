...
class PPO(object):
    def __init__(
        ...
        self.actor_p = ContinousPolicy(state_dim, action_p_dim, net_width).to(device)      
    ...
    def evaluate(self, state, stage):
        ...
        with torch.no_grad():
            if stage == 'sample':
                ...
                a = np.random.choice(pi.cpu().numpy().argpartition(-5)[-5:])
                return a, 1.0
            elif stage == 'push':
                state = state.reshape(1, -1).to(torch.float).to(device)
    ...
    def train(self):
        s, a_s, r_s, a_p, r_p, s_prime, old_prob_s, old_log_prob_p, terminal = self.make_batch()
        ...
        with torch.no_grad():
            ...
            adv_p, td_target_p = self.get_adv_td(vs, vs_, r_p, terminal)
        ...
        for _ in range(self.K_epochs):
            ...
            s, a_s, a_p, td_target_s, td_target_p, adv_s, adv_p, old_prob_s, old_log_prob_p = \
                s[perm].clone(), a_s[perm].clone(), a_p[perm].clone(), \
                td_target_s[perm].clone(), td_target_p[perm].clone(), \
                adv_s[perm].clone(), adv_p[perm].clone(), \
                old_prob_s[perm].clone(), old_log_prob_p[perm].clone()

            policy_s_loss,  policy_p_loss, value_loss = 0., 0., 0.
            ...
            for i in range(c_optim_iter_num):
                ...
                '''Sample Actor Update'''
                prob = self.actor_s.pi(s[index], softmax_dim=1)
                entropy = Categorical(prob).entropy().sum(0, keepdim=True)
                prob_a = prob.gather(1, a_s[index])
     
                ratio_s = torch.exp(torch.log(prob_a) - torch.log(old_prob_s[index]))  # a/b == exp(log(a)-log(b))
                surr1_s = ratio_s * adv_s[index]
                surr2_s= torch.clamp(ratio_s, 1 - self.clip_rate, 1 + self.clip_rate) * adv_s[index]
                actor_s_loss = - torch.min(surr1_s, surr2_s) - self.entropy_coef * entropy
                ...
                distribution = self.actor_p.get_dist(s[index])
                ...
                #print("adv_s:", adv_s[index].mean().item(), "adv_p:", adv_p[index].mean().item())
    ...
    def make_batch(self):
        s_lst, a_s_lst, r_s_lst, a_p_lst, r_p_lst, s_prime_lst, old_prob_s_lst, old_log_prob_p_lst, done_lst = [], [], [], [], [], [], [], [], []

        for transition in self.data:
            s, a_s, r_s, a_p, r_p, s_prime, old_prob_s, old_log_prob_p, done = transition
            s_lst.append(s)
            a_s_lst.append([a_s])
            r_s_lst.append([r_s])
            a_p_lst.append(a_p)
            ...
            old_prob_s_lst.append(old_prob_s)
        ...
        '''list to tensor'''
        with torch.no_grad():
            s, a_s, r_s, a_p, r_p, s_prime, old_prob_s, old_log_prob_p, done_mask = \
                torch.from_numpy(np.array(s_lst)).float().to(device), \
                torch.from_numpy(np.array(a_s_lst).astype(np.int64)).to(device), \
                ...
                torch.from_numpy(np.array(old_prob_s_lst)).float().to(device), \
                torch.from_numpy(np.array(old_log_prob_p_lst)).float().to(device), \
                torch.from_numpy(np.array(done_lst)).float().to(device), \

        return s, a_s, r_s, a_p, r_p, s_prime, old_prob_s, old_log_prob_p, done_mask
