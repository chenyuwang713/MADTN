...
def evaluate_policy(env, model, render):
    ...
    for j in range(turns):
        ...
        while step < total_t:
            ...
            aoi = env.edge_aoi
            selected_devices_id = np.argpartition(aoi, -env.device_num//2)[-env.device_num//2:] 
            a_s = np.zeros(env.device_num)
            a_s[selected_devices_id] = 1  # Select the two devices with the largest AoI
            end = time.perf_counter()
            s_prime, r_s, done, info = env.step(a_s)
            ep_r += r_s
            s = s_prime
            step += 1
        scores += ep_r / total_t
...
def main(): 
    ...
    for i in range(env.device_num):
        print("Device {}: {}".format(i, env.devices[i].get_id()))
        print("Device loss rate:", env.devices[i].get_loss_rate())
        print("Device delay:", env.devices[i].get_delay())
    ...
    while total_steps < Max_train_steps:
        ...
        while not done:
            ...
            s_prime, r_s, done, _ = env.step(a_s)
            s = s_prime
